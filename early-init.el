...
(defvar aquamacs-fonts
  `((default . ,(cond ((eq system-type 'darwin) "SF Mono")
    ...
    (fixed . ,(cond ((eq system-type 'darwin) "SF Mono")
                    ((eq system-type 'windows-nt) "SF Mono")
                    (t "Triplicate T4c")))
    (fixed-serif . ,(cond ((eq system-type 'darwin) "SF Mono")
                          ((eq system-type 'windows-nt) "SF Mono")
                          (t "Triplicate T4c")))
...
(defun aquamacs-load-charset-font (&optional font)
  ...
  (let ((default-font (or font (format "%s-%s:%s"
    ...
    (add-to-list 'face-font-rescale-alist `(,han-font . 1.0))
    (add-to-list 'face-font-rescale-alist `(,cjk-font . 1.0))
    ;; (add-to-list 'face-font-rescale-alist `(,variable-font . 1.0))
